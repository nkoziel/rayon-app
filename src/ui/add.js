...
export function addFromMedia(m, opts){
  ...
  if (LIB.entries.some(e => (e.al && e.al===m.id) || norm(e.t)===norm(m.titre))){
    toast(T("add.already")); return false;
  }
  LIB.entries.unshift({
    /* "manual" is a token, not a label: sourceLabel() turns it into text at render time, so a
       library built in one language reads correctly in the other. */
    id: uid(), t: m.titre, a: m.auteur, s: opts.source || "manual",
    st: m.statut || "", g: m.genres.slice(0,6),
  ...
  toast(T("add.added", { title: m.titre }));
  return true;
}
...
export function openAddModal(prefill){
  $("modalHost").innerHTML = `
    <div class="modal" id="modalScrim">
      <div class="modalbox" role="dialog" aria-label="${esc(T("add.title"))}">
        <div class="modalhead">
          <h3>${esc(T("add.title"))}</h3>
          <div class="row">
            <input type="search" id="addQ" placeholder="${esc(T("add.searchPlaceholder"))}" aria-label="${esc(T("add.searchLabel"))}" style="flex:1 1 auto">
            <button class="btn ghost" id="addClose">${esc(T("btn.close"))}</button>
          </div>
          <p class="rmeta" style="margin:8px 0 0">${esc(T("add.catalogue"))}</p>
        </div>
        <div id="addResults"><p class="note" style="margin:14px 15px">${esc(T("add.typeTwoLong"))}</p></div>
      </div>
    </div>`;
  const input = $("addQ");
  ...
  const run = async () => {
    ...
    if (term.length < 2){ $("addResults").innerHTML = `<p class="note" style="margin:14px 15px">${esc(T("add.typeTwo"))}</p>`; return; }
    const mine = ++seq;
    $("addResults").innerHTML = `<p class="loading" style="margin:14px 15px">${esc(T("add.searching"))}</p>`;
    try{
      const data = await gql(SEARCH_PAGE_Q, {s: term});
      if (mine !== seq) return;
      const hits = (data.Page.media||[]).map(shapeMedia).filter(Boolean);
      if (!hits.length){ $("addResults").innerHTML = `<p class="note" style="margin:14px 15px">${esc(T("add.noResults", { term }))}</p>`; return; }
      $("addResults").innerHTML = hits.map((h,i)=>{
        const owned = isOwned(h);
        const meta = [h.type, h.annee, h.chapitres?h.chapitres+" ch.":null, statusLabel(h.statut), h.score?h.score+"/100":null].filter(Boolean).join(" · ");
        return `<div class="hit">
          ${h.cover?`<img src="${esc(h.cover)}" alt="" loading="lazy">`:'<span class="ph"></span>'}
          <span style="min-width:0">
            <span class="ht">${esc(h.titre)}</span>${owned?`<span class="owned">${esc(T("owned.badge"))}</span>`:''}<br>
            <span class="hm">${esc(meta)}${h.auteur?" · "+esc(h.auteur):""}</span>
          </span>
          <span class="act"><button class="btn sm ${owned?'ghost':''}" data-add="${i}" ${owned?'disabled':''}>${esc(T(owned?"reco.added":"btn.add"))}</button></span>
        </div>`;
      }).join("");
      [...$("addResults").querySelectorAll("[data-add]")].forEach(b=>{
        b.onclick = () => { if (addFromMedia(hits[+b.dataset.add])){ b.textContent = T("reco.added"); b.disabled = true; b.classList.add("ghost"); } };
      });
    }catch(e){
      if (mine !== seq) return;
